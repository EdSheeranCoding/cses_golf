...
#define NC2_PRIME(N) (N) * (2 * (N) - 1) * ((N) - 1)

#define NATTACK(N) 4 * (N - 1) * (N - 2)
#define NATTACK_PRIME(N) 8 * (N) - 16

// starting values for n == 2
unsigned long long result = 6, n_combo = 6, n_attacking_positions = 0;
int main(int argc, char **argv)
{
    ...
    for (int i = 3; i <= n; i++) {
        n_combo += NC2_PRIME(i);
        n_attacking_positions += NATTACK_PRIME(i);
        result = n_combo - n_attacking_positions;
        printf("%llu\n", result);
    }
}
